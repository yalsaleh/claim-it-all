...
MIGRATE_URL="${DATABASE_MIGRATE_URL:?DATABASE_MIGRATE_URL required}"
APP_URL="${DATABASE_URL:?DATABASE_URL required}"
...
psql "${APP_URL}" -v ON_ERROR_STOP=1 -c "SHOW is_superuser;" | grep -qi 'off\|false\|no' || true
