#!/usr/bin/env node
/**
 * pnpm integrity:check — read-only production integrity checks.
 */
import { PrismaClient } from '@prisma/client';
import { parseAppEnvironment, policyForEnvironment } from '../../packages/platform/dist/index.js';

const prisma = new PrismaClient();
const findings = [];

function note(code, severity, message) {
...
async function main() {
  ...
  if (!lastBackup) {
    note('BACKUP_MISSING', 'warn', 'No successful BackupRun recorded');
  }

  const openSupport = await prisma.supportAccessAuditSession.count({ where: { active: true } });
  if (openSupport > 0) {
    note('SUPPORT_ACTIVE', 'info', `${openSupport} active support session(s)`);
  }
