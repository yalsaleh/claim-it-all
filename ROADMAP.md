...
### Slice 8 (done — Mode A)
`ConnectorAccount` + approved `ConnectorProjectScope` (`IMPORT_ONLY`); `ExternalRecord` identity; import via existing ingestion/outbox/ARQ; fake/local connectors CI-only; deterministic operational alerts and dashboards; portfolio respects membership. ADRs 087–102. Threat model: [docs/threat-models/connectors-and-operations.md](./docs/threat-models/connectors-and-operations.md). **No autonomous event confirmation, deadline activation, or notice dispatch.**

### Slice 9 (done — Mode A / evidence-based)
Production hardening documentation and `@contractradar/platform` primitives: environment classification, production validation, secret references, tenant admin / support access / break-glass **policy** (break-glass not in product UI), session hardening, structured logging, metrics/tracing, health/readiness, backup/restore (CI/MinIO only), DR tiers, migration safety, provider enablement checklist, kill switches, pilot readiness (non-waivable), tenant limits, retention/offboarding, supply-chain scan, deployment hardening. ADRs 103–123. **No real deployment, no real providers, fake blocked in PILOT/PRODUCTION.** Next phase: **controlled pilot deployment** (after evidence).
