...
Downloadable CI artifacts under `artifacts/**` with `if-no-files-found: error`; owned expiring vulnerability-exception register + `pnpm security:audit` gating; synthetic restore proves cross-tenant isolation, object byte/checksum round-trip, and tamper rejection; real Slice 8→9 forward upgrade rehearsal (not `migrate reset`); migration drift + runtime/migration role separation; container non-root + DI health gates; license inventory; `pnpm env:doctor` for local Docker/Postgres absence (**NOT RUN**, not PASS). See [SLICE9B_EVIDENCE.md](../SLICE9B_EVIDENCE.md). **Next phase: controlled pilot deployment (still no real provider onboarding until separately approved).**
